...
import { fetchSiteData, updateAccount, getAccountDetail } from '@/api'
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      Object.assign(formData, {
        ...detail,
        // 根据validUntil判断是否为永久有效
        validityType: detail.validUntil === '9999-12-31' ? 'permanent' : 'custom',
      })
    } else {
    ...
    if (valid) {
      try {
        ...
        const submitData = {
          ...
          id: route.params.id, // 添加账号ID
        }

        // 调用更新账号API
        const res = await updateAccount(submitData)
        if (res.code === 200) {
