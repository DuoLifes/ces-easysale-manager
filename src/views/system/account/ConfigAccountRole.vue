    ...
    <div class="form-container">
      ...
      >
        <div class="form-content">
          ...
          <el-form-item label="角色：" prop="roleNames" required>
            <el-select
...
import { fetchRoleData, getAccountDetail, updateAccountRole } from '@/api'
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      Object.assign(formData, {
        ...
        roleNames: detail.roleName ? detail.roleName.split(',') : [],
      })
    } else {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        // 调用更新账号角色API，将多个角色用逗号连接成字符串传递给后端
        const roleName = formData.roleNames.join(',')
        const res = await updateAccountRole(id, roleName)
        if (res.code === 200) {
